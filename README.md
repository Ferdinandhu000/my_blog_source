...
`dist/` 是完整静态站点。首次访问默认进入英文首页 `/en/`；设置中手动选择中文后会记住偏好，中文首页使用 `/`。首页默认启用 Super Performance（三维画面以 50% 分辨率、最高 60 帧运行，实际帧率取决于设备），音频关闭；设置中可以调整画质。档案卡出现时会预取正文，打开阅读窗时复用已加载的文章。若浏览器无法创建 WebGL 上下文，可从首页错误提示进入归档或搜索。
