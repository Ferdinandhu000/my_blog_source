...
const ARTICLE_INDEX = path.join(__dirname, '../src/generated/articles.json');
        ...
        if (disc.comments && disc.comments.nodes) {
            disc.comments.nodes.forEach(comment => {
                ...
                if (now - commentDate < 24 * 60 * 60 * 1000) {
                    ...
                    // Resolve the discussion pathname against the current Markdown index.
                    const cleanPath = articleTitle.replace(/^\/+|\/+$/g, '');
                    if (cleanPath.startsWith('p/')) {
                        const slug = cleanPath.substring(2);
                        if (fs.existsSync(ARTICLE_INDEX)) {
                            try {
                                const index = JSON.parse(fs.readFileSync(ARTICLE_INDEX, 'utf-8'));
                                articleTitle = index.records.find(article => article.slug === slug)?.title ?? articleTitle;
                            } catch (err) {
                                console.warn(`Failed to read article title for slug: ${slug}`, err);
                            }
                        }
                    }
