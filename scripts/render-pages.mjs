...
for (const article of articles) {
  for (const language of ['zh', 'en']) {
    ...
    const body = `<article class="article"><div class="eyebrow">${escape(version.category)} / ${escape(article.id)}</div><h1>${escape(version.title)}</h1><div class="meta"><time datetime="${article.date}">${article.date}</time><span>Ferdinand Hu</span></div>${tags ? `<div class="tags">${tags}</div>` : ''}<div class="prose">${rendered}</div>${giscus}</article>`;
...
write('en/about/index.html', shell('CV', 'Research CV', '<div class="unified-cv"><div><span class="unified-kicker">RESEARCH / CV</span><h1>CV</h1><p>Content in preparation</p></div></div>', { url: '/en/about/', language: 'en', kind: 'cv' }));
...
write('en/index.html', '<!doctype html><meta charset="utf-8"><script>try{const p=JSON.parse(localStorage.getItem("hj-blog-settings")||"{}");p.language="en";localStorage.setItem("hj-blog-settings",JSON.stringify(p))}catch{}location.replace("/")</script><a href="/">Open HJ Blog</a>');
write('sitemap.xml', `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${['/', '/archives/', '/search/', '/terminal/', '/about/', '/en/archives/', '/en/search/', '/en/terminal/', '/en/about/', ...articles.flatMap(a => [a.url, `/en${a.url}`])].map(url => `<url><loc>${origin}${url}</loc></url>`).join('')}</urlset>`);
